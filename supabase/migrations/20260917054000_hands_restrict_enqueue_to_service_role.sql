revoke execute on function public.hands_enqueue_command(text,text,jsonb) from public, anon, authenticated;
grant execute on function public.hands_enqueue_command(text,text,jsonb) to service_role;
...
begin
  ...
  where a.enabled and a.worker_id = p_worker_id;
  ...
  values (p_task_id,'queued',p_command,p_worker_id,v_channel_token)
